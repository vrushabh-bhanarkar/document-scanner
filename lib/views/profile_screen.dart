...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final email =
        sub.userEmail ?? FirebaseAuth.instance.currentUser?.email ?? 'Guest';
    final status = sub.isSubscribed ? 'Subscribed' : 'Free';
    final lastPayment = sub.lastPaymentDate != null
        ? DateFormat.yMMMd().format(sub.lastPaymentDate!)
        : '—';
          ...
          children: [
            ...
            if (!sub.isSubscribed)
              Padding(
                ...
                child: Text(
                    'You can buy a subscription to unlock unlimited scans and cloud sync.',
                    style: AppTextStyles.bodySmall),
              ),
          ],
        ),
